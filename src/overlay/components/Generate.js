...
const Generate = ({ generate, setPreview, typeActive, toneActive, lengthActive, handleGenerate, setStringResponse }) => {
    ...
    return (
        <>
            <button className={`bg-blue-700 hover:bg-blue-600 text-white p-3 rounded gap-2 w-full block text-center text-sm ${generate ? "cursor-not-allowed" : null}`} disabled={generate} onClick={handleGenerateFunc}>
                {generateState ? (
                    <ClipLoader
                        color="#fff"
                        size={22}
